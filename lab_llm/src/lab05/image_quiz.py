import base64  # 이미지 바이너리 데이터를 ASCII 텍스트로 인코딩.
...
def base64encode_image(image_path):
    # 이미지 바이너리 데이터를 읽어서 ASCII 텍스트로 인코딩된 문자열을 리턴.
...
def make_image_quiz(image_path):
    # 이미지 한 개를 GPT 서버로 전송해서 퀴즈 문제/정답/해설을 만들도록 요청을 보내고 응답 리턴.
    prompt = '''
    ...
    (2) 맨 앞의 사람은 노트북을 사용하고 있다.
    (3) 맨 앞의 사람은 빨간 셔츠를 입고 있다.
    (4) 점원은 커피를 만들고 있다.
    A. (2) - 맨 앞의 사람은 책을 읽고 있다.
    ...
    messages = [
        {
            ...
            'content': [
                ...
                {
                    'type': 'image_url',
                    'image_url': { 'url': f'data:image/jpg;base64,{b64_encoded}' }
                }
            ]
        }
    ]
...
def main():
    ...
    for i, path in enumerate(glob('./images/*.jpg')):  # 현재 폴더 아래의 images 폴더에 저장된 모든 jpg 파일을 찾음.
        try:
            ...
            # GPT 요청-응답 과정에서 에러가 발생하더라도, 다음 이미지를 이어서 퀴즈를 만들도록 하기 위해서.
            print(e)  # 에러 메시지 출력
            continue  # for 반복(next iteration)을 계속 수행.
