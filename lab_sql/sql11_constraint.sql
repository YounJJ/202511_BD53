/*
 * 제약조건(constraint): 데이터의 무결성을 유지하는 위한 조건들.
...
-- 위 29줄의 insert 문장을 다시 실행시키면 primary key 위배로 에러가 발생
...
-- 데이터를 삽입할 때 PK에 없는 값은 삽입되지 않도록 하기 위해서. (null 가능, unique 와 nut null과 상관 없음.)
-- (1) PK를 갖는 테이블을 먼저 생성, 그 PK를 참조하는 FK를 갖는 테이블을 나중에 생성.
--      예: dept 테이블을 먼저 생성, emp 테이블을 나중에 생성
 ...
 * (4) cust_gender: 고객 성별. 1자리 정수. 기본값 0. (0, 1, 2) 중 1개의 값만 가능.
